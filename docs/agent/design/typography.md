...
**Key Principle:** Use Tailwind utility classes for typography styling. All typography values are defined in `@kuat/core` and accessible via Tailwind classes.

---

## Font Families

Font families are defined in `@kuat/core/src/variables.css` and available via Tailwind utilities.
...
Text colors use the design system's semantic color tokens from `@kuat/core`. All colors support light and dark modes automatically.
...
2. **Maintain consistency**
   ...
   - ✅ Use design tokens from `@kuat/core`
...
- **Tokens:** All values come from `@kuat/core/src/variables.css`
...
- **Design tokens:** All typography values reference tokens from `@kuat/core`
